...
const Vender_productEdit = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { venderPoductId } = useParams();

  const { VenderProdcutGetByID } = useSelector(
    (state) => state.VenderProdcutGetByIDRecuer
  );

  useEffect(() => {
    if (VenderProdcutGetByID) {
      settitle(VenderProdcutGetByID?.title);
      setdescription(VenderProdcutGetByID?.description);
      setcontent(VenderProdcutGetByID?.product_link);

      setFiles(VenderProdcutGetByID?.vendor_product_detail?.images);
    }
  }, [VenderProdcutGetByID]);
      ...
      {loadingloader ? (
        ...
        <div class="pl-6">
          <div className="main-page-div bg-green-200">
            <h1 className="heading-top">Shop Edit</h1>
...
export default Vender_productEdit;
