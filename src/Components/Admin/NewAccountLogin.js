...
import { NewUserLoginListAction } from "../../Redux/actions/UserAction";

function NewAccountLogin() {
  const dispatch = useDispatch();

  const { NewUserLoginList } = useSelector(
    (state) => state.NewUserLoginListReducer
  );

  const [selectWeek, setSelectWeek] = useState(2);

  useEffect(() => {
    dispatch(NewUserLoginListAction(selectWeek));
  }, [selectWeek]);
  ...
  useEffect(() => {
    let userData = [];
    NewUserLoginList?.map((item, index) => {
      ...
      <div>
        <div className="">
          <div className="main-page-div bg-green-200">
            <h1 className="heading-top">New Register Users</h1>
